...
/**
 * Improve CSS discovery / FOUC for Laravel-mounted Nicepage pages.
 * - Converts leftover preload-as-style tags to blocking stylesheets
 * - Hoists stylesheets to the top of <head> with absolute hrefs (before large scripts)
 * - Injects a tiny branded loading shell (inline) so first paint is dark brand, not unstyled HTML
 *
 * @param string $html
 * @return string
 */
function legacy_optimize_css_loading($html) {
	if (!is_string($html) || $html === '' || stripos($html, '<head') === false) {
		return $html;
	}

	// Convert Nicepage-style async CSS (preload + onload) to blocking stylesheets.
	$html = preg_replace_callback(
		'#<link\b[^>]*>#i',
		function ($m) {
			$tag = $m[0];
			$isPreloadStyle = (bool) preg_match('/\brel=["\']preload["\']/i', $tag)
				&& (
					preg_match('/\bas=["\']style["\']/i', $tag)
					|| preg_match('/onload=["\'][^"\']*stylesheet/i', $tag)
				);
			if (!$isPreloadStyle) {
				return $tag;
			}
			$tag = preg_replace('/\brel=["\']preload["\']/i', 'rel="stylesheet"', $tag);
			$tag = preg_replace('/\bas=["\']style["\']/i', '', $tag);
			$tag = preg_replace('/\bonload=["\'][^"\']*["\']/i', '', $tag);
			$tag = preg_replace('/\bonerror=["\'][^"\']*["\']/i', '', $tag);
			if (!preg_match('/\btype=/i', $tag)) {
				$tag = preg_replace('/<link\b/i', '<link type="text/css"', $tag, 1);
			}
	...
	$headOpen = $headMatch[1];
	$headInner = $headMatch[2];
	$headClose = $headMatch[3];

	$cssLinks = array();
	$headInner = preg_replace_callback(
		'#<link\b(?=[^>]*\brel=["\']stylesheet["\'])[^>]*>\s*#i',
		function ($m) use (&$cssLinks) {
			$cssLinks[] = trim($m[0]);
			return '';
		},
		$headInner
	...
	$baseUrl = function_exists('getBaseUrl') ? getBaseUrl() : (isset($GLOBALS['LEGACY_BASE_URL']) ? $GLOBALS['LEGACY_BASE_URL'] : '/site/');
	...
	foreach ($cssLinks as $tag) {
		// Ensure truly render-blocking stylesheets (clear leftover media/onload tricks).
		$tag = preg_replace('/\bmedia=["\'][^"\']*["\']/i', '', $tag);
		$tag = preg_replace('/\bonload=["\'][^"\']*["\']/i', '', $tag);
		$tag = preg_replace('/\s{2,}/', ' ', trim($tag));
		$tag = preg_replace_callback(
			'/\bhref=(["\'])([^"\']+)\1/i',
			function ($hm) use ($baseUrl) {
				$href = $hm[2];
				$isAbsolute = (strpos($href, '//') === 0)
					|| (isset($href[0]) && ($href[0] === '/' || $href[0] === '#' || $href[0] === '?'))
					|| preg_match('/^[a-z][a-z0-9+.-]*:/i', $href);
				if (!$isAbsolute) {
					$href = $baseUrl . $href;
				}
				return 'href=' . $hm[1] . $href . $hm[1];
			},
			$tag
		);
		$blockingLinks[] = $tag;
	...
	// Early background + blocking CSS discovery before large head scripts.
	// Soft branded ring hides on DOMContentLoaded (styles already applied via blocking links).
	$early = "\t<style id=\"jk-early-css\">"
		. "html{background-color:#0d0404}"
		. "html.jk-loading,html.jk-loading body{background-color:#0d0404!important}"
		. "#jk-page-loader{position:fixed;inset:0;z-index:2147483000;display:flex;align-items:center;justify-content:center;"
		. "background:#0d0404;transition:opacity .25s ease,visibility .25s ease}"
		. "#jk-page-loader .jk-ring{width:40px;height:40px;border:3px solid rgba(12,163,166,.35);"
		. "border-top-color:#dfc91b;border-radius:50%;animation:jk-spin .7s linear infinite}"
		. "@keyframes jk-spin{to{transform:rotate(360deg)}}"
		. "html:not(.jk-loading) #jk-page-loader{opacity:0;visibility:hidden;pointer-events:none}"
		. "</style>\n"
		. "\t" . implode("\n\t", $blockingLinks) . "\n"
		. "\t<script data-custom-script=\"true\">"
		. "document.documentElement.className+=(document.documentElement.className?' ':'')+'jk-loading';"
		. "(function(){function d(){var h=document.documentElement;if(!h||h.__jkReady)return;h.__jkReady=1;"
		. "h.className=(' '+h.className+' ').replace(/ jk-loading /g,' ').replace(/^\\s+|\\s+$/g,'');"
		. "var e=document.getElementById('jk-page-loader');"
		. "if(e){setTimeout(function(){if(e&&e.parentNode)e.parentNode.removeChild(e);},280);}}"
		. "if(document.readyState==='loading')document.addEventListener('DOMContentLoaded',d);else d();"
		. "window.addEventListener('load',d);setTimeout(d,6000);})();"
		. "</script>\n";

	$newHead = $headOpen . "\n" . $early . $headInner . $headClose;
	$html = str_replace($headMatch[0], $newHead, $html);

	if (stripos($html, 'id="jk-page-loader"') === false) {
		$html = preg_replace(
			'/(<body\b[^>]*>)/i',
			'$1<div id="jk-page-loader" aria-hidden="true"><div class="jk-ring"></div></div>',
			$html,
			1
		);
	}
